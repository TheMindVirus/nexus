...
def reload():
    global filename, filelist, pathlist, x10dlist
    ...
    if command == "search":
        ...
        for file in filelist:
            if search(term, file):
                ...
                data += os.path.join(here, pathlist[file]) + x10dlist[file] + "\n"
        data = data.replace("\\", "/")
        with open(indexfile, "w") as index:
            index.write(data)
        sys.stderr.write("Index Written to " + str(indexfile) + "\n")
    elif command == "help":
        help2()
    else:
